...
# Times New Roman font without LaTeX
plt.rcParams.update({
    "font.family": "serif",
...
def write_metrics_table(custom_json, default_json, outdir):
    rows = [
        ("Avg JCT (s)", "avg_jct"),
    ...
    lines = [
        "\\begin{tabular}{lcc}",
        "\\toprule",
        "Metric & Custom & Default \\\\",
        "\\midrule"
    ]
    for label, key in rows:
        c = custom_json.get(key, "--")
        d = default_json.get(key, "--")
        lines.append(f"{label} & {c:.2f} & {d:.2f} \\\\")
...
def plot_cdf(metric_key, label, ylabel, custom_vals, default_vals, outdir):
    ...
    write_caption_snippet(
        ...
        title=f"Cumulative distribution of {label.lower()} comparing the custom and default schedulers.",
        label=f"cdf-{metric_key}",
        outdir=outdir
    )


def plot_summary_bars(custom_json, default_json, outdir):
    ...
    plt.figure()
    bars1 = plt.bar(x - width / 2, custom_vals, width, label="Custom")
    bars2 = plt.bar(x + width / 2, default_vals, width, label="Default")
    ...
    plt.ylabel("Seconds")
    plt.title("Summary Metrics: Custom vs Default")
    ...
    write_caption_snippet(
        filename=str(outpath),
        title="Summary comparison of key performance metrics of the custom and default scheduler.",
        label="summary-bars",
        outdir=outdir
    )
...
def main():
    ...
    plot_cdf("jct", "Job Completion Time (s)", "CDF", custom_jct, default_jct, args.outdir)
    plot_summary_bars(custom_json, default_json, args.outdir)
    write_metrics_table(custom_json, default_json, args.outdir)
